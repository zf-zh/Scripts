...
DAEMON_PATH="/etc/systemd/system/fdu-connect-daemon-helper.service"
...
generate_service() {
    if [ $DAEMON_TYPE -eq 1 ]
    then
        # Generate customized service
        cat <<EOF > "$DAEMON_PATH"
[Unit]
Description=fdu-connect daemon helper
...
EOF
        if [ $CONFIG_TYPE -eq 2 ]
        then
            echo "ExecStart=${EXEC_PATH} $EXEC_ARGS" >> "$DAEMON_PATH"
        else
            echo "ExecStart=${EXEC_PATH} --config ${CONFIG_PATH}" >> "$DAEMON_PATH"
        fi
        cat <<EOF >> "$DAEMON_PATH"
Restart=on-failure
RestartSec=5
...
EOF
        if [ $? -ne 0 ]
        then
            echo "$0: Failed to create service file \"$DAEMON_PATH\"" >&2
            return 1
        fi
    else
        # Use default service
        ...
        if [ $? -ne 0 ]
        then
            echo "$0: Failed to copy default service file \"$DAEMON_PATH_DEFAULT\" to \"$DAEMON_PATH\"" >&2
            return 1
        fi
    fi
    return 0
...
start_daemon() {
    ...
    if systemctl -q is-active "$DAEMON_NAME"
    then
